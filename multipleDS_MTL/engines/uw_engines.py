...
class LossCalculator:
    ...
    def general_loss(self, output_losses, weight_method=None):
        ...
        # losses = {k: 1 / (2 * torch.exp(weight_method[k])) * v + weight_method[k] / 2 for k, v in task_per_loss.items()}
        losses = weight_method(task_per_loss)
        
        return losses


# TODO: 중복적인 변수들 정리 필요함.
def training(model, optimizer, data_loaders, 
    ...
    for i, b_data in enumerate(biggest_dl):
        ...
        # final goal losses
        uw_losses = loss_calculator.loss_calculator(loss_dict, model.module.uw)
        total_loss = sum(loss for loss in uw_losses.values())
        if not math.isfinite(total_loss):
            logger.log_text(f"Loss is {total_loss}, stopping training\n\t{loss_dict}\n\t{uw_losses}", level='error')
            sys.exit(1)
            
        loss_dict.update({f"UW_{k}": v for k, v in uw_losses.items()})      
        loss_dict_reduced = metric_utils.reduce_dict(loss_dict, average=False)
        
        list_losses = list(loss_dict_reduced.values())
        list_losses.append(total_loss)
        ...
        if scaler is not None:
            ...
            print("*"*40, "Iteration", i, "*"*40)
            print("Before clipping")
            for k, p in model.module.named_parameters():
                # print(k, torch.min(p.grad), torch.max(p.grad))
                if p.grad is not None:
                    if torch.any(torch.isinf(p.grad)):
                        views = torch.sort(p.grad.view(-1))
                        print(views)    
                        print("INF module:", k) 
                        exit()
            ...
            if args.grad_clip_value is not None:
                ...
                # (p.grad, min=min_grad, max=max_grad)
                # torch.nn.utils.clip_grad_norm_([p for p in model.parameters() if p.requires_grad],
                #                                              args.grad_clip_value)
                
                # for p in model.parameters():
                #     if p.requires_grad and p.grad is not None:
                #         if torch.any(torch.isinf(p.grad)): clip_fucntion = torch.clamp_
                #         else: clip_fucntion = torch.nn.utils.clip_grad_norm_
        ...
        uw_losses_reduced = metric_utils.reduce_dict(uw_losses, average=False)
        metric_logger.update(main_lr=optimizer.param_groups[0]["lr"])
        metric_logger.update(loss=sum(l for l in uw_losses_reduced.values()), **loss_dict_reduced)
        iter_time.update(time.time() - end) 
        
        if BREAK:
            args.print_freq = 10
        
        logsigma = {k: w.data for k, w in model.module.uw.logsigma.items()}
        ...
        if tb_logger:
            ...
            tb_logger.update_scalars(logsigma, i)
            '''
