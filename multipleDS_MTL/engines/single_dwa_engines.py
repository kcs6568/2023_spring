...
def training(model, optimizer, data_loaders, 
    ...
    dwa = model.weighting_method
    dwa.set_epoch(epoch)
    dwa_epoch = dwa.is_valid_epoch()
    task_iter_loss = {k: [] for k in datasets}
    ...
    for i, b_data in enumerate(biggest_dl):
        ...
        for data in datasets: 
            data_loss = sum(loss for k, loss in loss_dict.items() if data in k).detach()
            task_iter_loss[data].append(data_loss)
        
        if dwa_epoch:
            dwa_losses = loss_calculator.loss_calculator(loss_dict, model.module.dwa)
            loss_dict.update({k: v for k, v in dwa_losses.items()})      
        else: 
            dwa_losses = loss_dict
        
        total_loss = sum(loss for loss in dwa_losses.values())
        if not math.isfinite(total_loss):
            logger.log_text(f"Loss is {total_loss}, stopping training\n\t{loss_dict}\n\t{dwa_losses}", level='error')
            sys.exit(1)
        ...
        metric_logger.update(loss=total_loss, **loss_dict_reduced)
    ...
    avg_loss = {k: 0. for k in datasets}
    
    for k, avg in task_iter_loss.items(): avg_loss[k] = torch.stack(avg).mean()
    dwa.save_loss_to_buffer(avg_loss)
