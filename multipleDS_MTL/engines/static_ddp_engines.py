    ...
    for i, b_data in enumerate(biggest_dl):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            ...
            for ii, dset in enumerate(datasets):
                other_args = {'dataset': dset}
                loss_dict = all_models(input_set[dset], other_args)
                per_task_loss = sum(loss for loss in loss_dict.values())
                task_output_loss.update({dset: per_task_loss})
                logged_loss.update(loss_dict)
        
        if wm is not None:
            task_output_loss = wm(task_output_loss)
            logged_loss.update({f"{wm.method_name}_{n}": l for n, l in task_output_loss.items()})
        else:
            if loss_calculator is not None:
                _, task_output_loss = loss_calculator.loss_calculator(task_output_loss)
                logged_loss.update({f"bal_{n}": l for n, l in task_output_loss.items()})
        
        loss_dict_reduced = metric_utils.reduce_dict(logged_loss, average=False)
        losses_reduced = sum(loss for loss in loss_dict_reduced.values())
        loss_value = losses_reduced.item()
        if not math.isfinite(loss_value):
            logger.log_text(f"Loss is {loss_value} at the {i} iteration, stopping training\n\t{loss_dict_reduced}", level='error')
            sys.exit(1)
        ...
        if scaler is not None:
            ...
            scaler.scale(task_output_loss[all_models.base_dataset]).backward(retain_graph=True)
            sum_loss += task_output_loss[all_models.base_dataset]
            
            base_grad = {}
            for n, p in all_models.get_shared_encoder.named_parameters():
                base_grad.update({n: p.grad})
            
            for d in datasets:
                if d == all_models.base_dataset: continue
                for name, param in all_models.get_network(d).encoder.named_parameters():
                    param.grad = base_grad[name]   
                scaler.scale(task_output_loss[d]).backward(); sum_loss += task_output_loss[d]
                
                
            # for task, taskloss in task_output_loss.items():
            #     if task == all_models.base_dataset: continue
            #     taskloss.backward(); sum_loss += taskloss
            
            clip_targets = []
            for dset in datasets:
                task_p = []
                if dset == all_models.base_dataset: task_p = [p for p in all_models.get_network(dset).parameters() if p.requires_grad]
                else: task_p = [p for n, p in all_models.get_network(dset).named_parameters() if p.requires_grad and (not 'encoder' in n)]
                clip_targets.extend(task_p)
            
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_( # clip gradient values to maximum 1.0
                clip_targets, args.grad_clip_value)
            
            
            # all_grad = []
            # for d, loss in task_output_loss.items(): 
            #     assert loss.dtype is torch.float32
            #     scaler.scale(loss).backward()
                
                
            #     g = 0.
            #     if d != all_models.base_dataset:
            #         for src, tar in zip(
            #             all_models.get_shared_encoder.parameters(),
            #             all_models.get_network(d).encoder.parameters()):
                        
            #             tar.grad = src.grad.clone()
                        
            #     else:
            #         for p in all_models.get_network(d).parameters():
            ...
            norms = []
            snorm = []
            for dset, loss in task_output_loss.items():
                # print(dset, loss)
                loss.backward()
                ...
                task_norm = []
                sum_norm = 0
                for n, p in all_models.get_network(dset).encoder.named_parameters():
                    nm = p.grad.norm()
                    # print(n, "||| Grad sum and norm:", p.grad.sum(), p.grad.norm())
                    task_norm.append(nm)
                    sum_norm += nm
                    
                snorm.append(sum_norm)
                task_norm = torch.stack(task_norm).unsqueeze(0)
                norms.append(torch.transpose(task_norm, 0, 1))
            ...
            all_grad = []
            task_output_loss[all_models.base_dataset].backward(retain_graph=True)
            sum_loss += task_output_loss[all_models.base_dataset]
            base_grad = {}
            for n, p in all_models.get_shared_encoder.named_parameters():
                base_grad.update({n: p.grad})
            ...
            for d in datasets:
                ...
                for name, param in all_models.get_network(d).encoder.named_parameters():
                    param.grad = base_grad[name]   
                task_output_loss[d].backward(); sum_loss += task_output_loss[d]
        ...
        dist.all_reduce(sum_loss)
        metric_logger.update(main_lr=optimizer.param_groups[0]["lr"])
        metric_logger.update(loss=sum_loss, **loss_dict_reduced)
        ...
        if wm is not None:
            logsigma = {k: w.data for k, w in all_models.weight_method.logsigma.items()}
        if (i % args.print_freq == 0 or i == (biggest_size - 1)) and get_rank() == 0:
            ...
            if wm is not None:
                logsigma = {k: w.data for k, w in all_models.weight_method.logsigma.items()}
                logger.log_text(f"Uncertainty Weights: {logsigma}")
            
        if tb_logger:
            tb_logger.update_scalars(loss_dict_reduced, i)   
            if wm is not None: tb_logger.update_scalars(logsigma, i)
