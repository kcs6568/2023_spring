    ...
    for i, b_data in enumerate(biggest_dl):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            for ti, dset in enumerate(datasets):
                task_loss = module(input_set[dset], dset)
        ...
        metric_logger.update(loss=losses/3, **loss_dict_reduced)
        ...
        if (i % args.print_freq == 0 or i == (biggest_size - 1)) and get_rank() == 0:
            ...
            if not module.retrain_phase:
                ...
                for dset in datasets: logger.log_text(f"{dset}:\n{torch.transpose(module.task_gating_params[dset].data.clone().detach(), 0, 1)}")
                logger.log_text(f"Temperature: {module.decay_function.temperature}\n")
    ...
    for dataset, taskloader in data_loaders.items():
        ...
        for i, data in enumerate(taskloader):
            ...
            macs, _, outputs = get_model_complexity_info(
                model, batch_set[dataset][0], dataset, task, as_strings=False,
