...
class LossCalculator:
    ...
    def general_loss(self, output_losses, weight_method=None):
        assert isinstance(output_losses, dict)
        task_per_loss = {}
        for data in self.data_cats: task_per_loss.update({data: sum(loss for k, loss in output_losses.items() if data in k)})
        losses = weight_method(task_per_loss)
...
def training(model, optimizer, data_loaders, 
    ...
    norm_clip = False
    logger.log_text(f"The use of norm clipping: {norm_clip}")
    ...
    for i, b_data in enumerate(biggest_dl):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            ...
            for ii, dset in enumerate(datasets):
                ...
                task_output_loss.update({dset: sum_loss})
        ...
        if scaler is not None:
            ...
            for d, loss in task_output_loss.items(): 
                ...
                # for dd in datasets:
                #     if dd in got_data: continue
                #     for p in all_models.get_network(dd).parameters():
                #         if p.grad is not None:
                #             print(f"Dataset {dd} got grad.")
            ...
            if args.grad_clip_value is not None:
                ...
                for p in all_models.parameters():
                ...
                #     [p for p in all_models.parameters() if p.requires_grad],
                #     args.grad_clip_value)
                # if epoch == 0 and i < 4:
                #     for p in all_models.parameters():
                #         if p.requires_grad and p.grad is not None: 
                #             # inf_checker.append(torch.any(torch.isinf(p.grad)))
                #             torch.clamp_(p.grad, min=min_grad, max=max_grad)
                            
                # else:
                #     torch.nn.utils.clip_grad_norm_( # clip gradient values to maximum 1.0
                #     [p for p in model.parameters() if p.requires_grad],
                #     args.grad_clip_value)
                    ############# hard parameter sharing clipping in DDP #################
                    # task_params = []
                    # task_params.extend([p for p in all_models.get_task_stem(dset).parameters() if p.requires_grad])
                    # task_params.extend([p for p in all_models.get_task_head(dset).parameters() if p.requires_grad])
                    # encoder_params = [p for p in all_models.get_shared_encoder.parameters() if p.requires_grad]
                    # encoder_params.extend(task_params)
                    # torch.nn.utils.clip_grad_norm_( # clip gradient values to maximum 1.0
                    #     encoder_params, args.grad_clip_value)
                    # ######################################################################
                    
                    # base_encoder_grad = {k: p.grad for k, p in all_models.get_shared_encoder.named_parameters() if p.requires_grad}
                    # for dset in datasets:
                    #     if dset == all_models.base_dataset: continue
                    #     for k, p in all_models.get_network(dset).encoder.named_parameters():
                    #         p.grad = base_encoder_grad[k]
                            
            # task_norm = []
            # for dd in datasets:
            #     norm = 0
            #     for n, p in all_models.get_network(dd).encoder.named_parameters():
            #         norm += p.grad.norm()
            ...
            #     p_norm = 0.
            #     s_norm = 0.
            #     for n, p in all_models.get_network(dd).named_parameters():
            #         if 'encoder' in n: p_norm += p.norm()
            #         else: s_norm += p.norm()
                    
            #     task_param_norm.append(p_norm)
            #     task_specific_norm.append(s_norm)
            
            # print(task_param_norm)
            # print(task_specific_norm)
            # exit()
            # print("|||"*60)
            # print(); print()
        ...
        # task_weight = {k: w.data for k, w in wm.task_parameter.items()}
        if (i % args.print_freq == 0 or i == (biggest_size - 1)) and get_rank() == 0:
            ...
            # logger.log_text(f"Task Weight: {task_weight} / Global: {wm.global_parameter.data}")
