...
KILL_PROC="kill $(ps aux | grep dwa_train.py | grep -v grep | awk '{print $2}')"
TRAIN_FILE=dwa_train.py
...
CFG_PATH=$TRAIN_ROOT/cfgs/three_task/dwa/cifar10_minicoco_voc/$YAML_CFG

SCH="cosine"
OPT="adamw"
LR="1e-4"
GAMMA="0.1"
ADD_DISC="baseline_direct_clip_UnifyActivation"
        ...
        do
            ...
            do
                ...
                CUDA_VISIBLE_DEVICES=$DEVICES torchrun --nproc_per_node=$4 --master_port=$1 \
                    ...
                    --lr-scheduler $sch --opt $opt --lr $lr --gamma $gamma --grad-clip-value 1
